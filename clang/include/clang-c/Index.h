...
/**
 * Represents a specific kind of unary operator which can appear at a cursor.
 */
enum CX_UnaryOperatorKind {
  ...
  CX_UO_Extension = 13,
  CX_UO_Coawait = 14,
  CX_UO_LAST = CX_UO_Coawait
};

/**
 * \brief Returns the operator code for the unary operator.
 */
CINDEX_LINKAGE enum CX_UnaryOperatorKind
clang_Cursor_getUnaryOpcode(CXCursor C);

/**
 * \brief Returns a string containing the spelling of the binary operator.
 */
CINDEX_LINKAGE CXString
clang_Cursor_getUnaryOpcodeStr(enum CX_UnaryOperatorKind Op);
